...
func TestInsert(t *testing.T){
	if binaryTree.data.name != "NODE ZERO" {
		t.Errorf("NODE ZERO should be saved in the root node")
	}
	if binaryTree.leftChild.data.name != "LEFT CHILD"{
		t.Errorf("LEFT CHILD should be saved in the left child of the root node")
	}
	if binaryTree.rightChild.data.name != "RIGHT CHILD"{
		t.Errorf("RIGHT CHILD should be saved in the right child of the root node")
	}
}
